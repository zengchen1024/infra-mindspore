...
reset_image(){
    local image=$1
    kustomize edit set image swr.cn-north-4.myhuaweicloud.com/opensourceway/robot/robot-github=$image
}
...
cat << EOF >> $file
      - name: ${namespace}-${bot}
        endpoint: http://service-${bot}.${namespace}.svc.cluster.local:8888/github-hook
        events:
EOF
...
gen_deploy_yaml(){
    ...
    cp -r ../../deploy-robot-github/template/. .
    ...
    register_bot $bot "$events" $namespace ../../robot-github/hook-dispatcher/configmap.yaml
